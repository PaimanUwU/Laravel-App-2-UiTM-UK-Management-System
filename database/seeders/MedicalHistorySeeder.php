...
class MedicalHistorySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if ($patients->isEmpty() || $doctors->isEmpty()) {
            $this->command->error(
                "No patients or doctors found in database. Please create basic data first.",
            );
            return;
        }
        ...
        for ($i = 0; $i < 200; $i++) {
            try {
                ...
                $this->command->error(
                    "Error creating record {$i}: " . $e->getMessage(),
                );
                continue;
            }
        }
        ...
        $this->command->info(
            "Successfully created {$createdCount} medical history records!",
        );
    }
}
