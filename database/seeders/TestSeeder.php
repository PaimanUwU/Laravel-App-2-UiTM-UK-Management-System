...
class TestSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        Appointment::factory()
            ->count(20)
            ->create()
            ->each(function ($appt) use ($medications) {
                $type = rand(1, 3);
                if ($type == 1) {
            ...
            for ($i = 0; $i < 100; $i++) {
                try {
                    ...
                    DB::table("medical_checkups")->insert([
                        ...
                        "checkup_treatment" =>
                            $treatments[array_rand($treatments)],
                        "vital_bp" => $vitalBp,
                    ...
                    if (rand(1, 100) <= 30) {
                        ...
                        $mcStartDate = now()
                            ->subDays(rand(1, 30))
                            ->format("Y-m-d");
                        $mcEndDate = \Carbon\Carbon::parse($mcStartDate)
                            ->addDays($mcDays - 1)
                            ->format("Y-m-d");
                    ...
                    if (rand(1, 100) <= 40) {
                        ...
                        $availableMeds = DB::table("medications")
                            ->limit(10)
                            ->get();
                    ...
                    echo "Error creating medical record {$i}: " .
                        $e->getMessage() .
                        "\n";
                    continue;
                }
                ...
                try {
                    ...
                    if ($status === "DISCHARGED") {
                        ...
                        DB::table("medical_checkups")->insert([
                            "appt_ID" => $appointmentId,
                            "checkup_symptom" =>
                                $symptoms[array_rand($symptoms)],
                            "checkup_test" => $tests[array_rand($tests)],
                            "checkup_finding" =>
                                $findings[array_rand($findings)],
                            "checkup_treatment" =>
                                $treatments[array_rand($treatments)],
                            "vital_bp" => $vitalBp,
                            "vital_heart_rate" => $vitalHeartRate,
                    ...
                    echo "Error creating past appointment {$i}: " .
                        $e->getMessage() .
                        "\n";
                    continue;
                }
